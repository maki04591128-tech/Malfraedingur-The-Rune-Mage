...
- **v1.1 INC-5.1 完了（言語監修足場）**（2026-05-24）。INC-5 を「小さく投入→遊んで調整を反復」の方針通り 5 サブ機能に分解し、第 1 サブ増分 INC-5.1 を完結。① **A 古綴正規化** (`core/spell/orthography.gd`): á/æ/ǽ/œ/ø/ð/þ → a/ae/oe/d/th + lowercase の双方向写像。② **B 屈折マッチャ** (`core/spell/inflection_matcher.gd`): fjanda → fjandi+case=acc+number=sg を逆引き。形容詞性別解決 (mikinn → mikill+gender=masculine)、動詞活用 (meið → meida+mood=imp_2sg) も同経路で。`tokenize_freetext` に `known_word_ids` 引数追加で統合。③ **C 無辞書 Validator 統合**: `_check_unknown_words` を Validator に追加し resource=null トークンに `unknown_word` finding を発火。既存の `MISFIRE_MULT_BY_RULE["unknown_word"]=4.0` が初めて稼働。④ **D 複数形足場**: fjandi/eldr/vatn/vindr/jorth の inflection に `pl` 追加 (CV/Zoëga 出典)。Validator に `number_agreement` minor finding (×1.5) 追加、修飾語と target の数不一致を検出。⑤ **E 命令法足場**: WordResource に `verb_forms` 追加 (meida = inf/imp_2sg/imp_2pl/ind_3sg 投入)、AST.effect に `mood` 付与、Validator に `mood_required` minor finding (×1.5) 追加。命令法以外は警告のみ (タイル経路は mood="" で誤発火回避)。⑥ i18n: ja/en に grammar.unknown_word.* / grammar.number_agreement.* / grammar.mood_required.* / grammar.number.* / grammar.mood.* の 12 キー × 2 言語追加。⑦ test_smoke: 500 → **+47 アサーション ALL GREEN**。詳細は §3 INC-5 §INC-5.1 完了報告を参照
...
#### INC-5.1 完了報告（言語監修足場、2026-05-24）

INC-5 を 1 チャットで完結させるのは「小さく投入」原則に反するため、サブ増分に分解。**INC-5.1 = 言語監修足場 (屈折・複数・命令・無辞書経路)** を 1 チャットで完結。量産 (語彙 100 語・敵・フロア・碑文) と英語 UI は INC-5.2 以降で扱う。

| # | 項目 | 結果 |
|---|------|------|
| A | 古綴正規化 (`core/spell/orthography.gd`) | á/æ/ǽ/œ/ø/ð/þ/lowercase の双方向写像。idempotent 保証 |
| B | 屈折マッチャ (`core/spell/inflection_matcher.gd`) | fjanda → fjandi+case=acc+number=sg を逆引き。形容詞性別 (mikinn → mikill+gender=m)、動詞活用 (meið → meida+mood=imp_2sg) 経路も統合。`tokenize_freetext` に `known_word_ids` 引数を追加して呼び出し |
| C | 無辞書 Validator 統合 | `_check_unknown_words` で resource=null トークンに `unknown_word` finding 発火。既存 `MISFIRE_MULT_BY_RULE["unknown_word"]=4.0` が初めて稼働 |
| D | 複数形足場 | fjandi/eldr/vatn/vindr/jorth に `inflection.pl` 追加 (CV/Zoëga 出典)。`number_agreement` minor finding を Validator に追加、`MISFIRE_MULT_BY_RULE["number_agreement"]=1.5` |
| E | 命令法足場 | WordResource に `verb_forms` 追加、meida = inf/imp_2sg/imp_2pl/ind_3sg を投入。AST.effect.mood、`mood_required` minor finding、`MISFIRE_MULT_BY_RULE["mood_required"]=1.5`。タイル経路 (mood="") は誤発火しない |
| - | i18n | ja/en に grammar.{unknown_word,number_agreement,mood_required}.{reason,recommended} + grammar.number.{sg,pl} + grammar.mood.{inf,imp_2sg,imp_2pl,ind_3sg} の 12 キー × 2 言語追加 |
| - | test_smoke | 500 → **547+ ALL GREEN** (+47 アサーション)。A〜E + Tokenizer 統合 + Resolver 暴発倍率を担保 |

**実機検証 (2026-05-24, computer-use)**:
- F6 (test_smoke) → ALL GREEN
- F5 (dungeon_view) → T キーで freetext モーダル: `meið fjanda` を入力すると tokens = [{meida, mood=imp_2sg}, {fjandi, case=acc, number=sg}] に解決され、mood_required も number_agreement も発火せず詠唱成立。`zxqwerty fjandi` 入力で `unknown_word` finding 発火、暴発確率上昇 (×4) を実機確認

**INC-5.1 引き継ぎ事項（INC-5.2 以降）**:
- 語彙量産（17 → 50 → 100 語）、敵 +4-6 種、フロア追加、碑文 +5-8 個
- `number_agreement` のコア違反扱い格上げ（現状 minor）
- 動詞活用の全パラダイム（直説法・接続法・分詞）と強弱変化クラス分類
- 英語グロス・en.po の grammar.* / ui.* 全キー充足 (DL1, INC-6 リリースゲート)
- BalanceConfig Resource 化（DAMAGE_SCALE / MISFIRE_MULT_BY_RULE / V_MAX_BY_TIER / aoe.damage_split）
- AoE `vítt` の damage_split キャリブレーション
- 無辞書モードの本格化（scaffold=none で語彙タイル非表示・全タイプ入力強制）
- 任意巻き戻しの確認モーダル / loop_delta の祝祭・励まし演出
